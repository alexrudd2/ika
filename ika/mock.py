...
class Hotplate(RealHotplate):
    ...
    async def query(self, query):
        ...
        async with self.lock:  # lock releases on CancelledError
            ...
            elif query == self.READ_DEVICE_TYPE:
                return self.state["info"]["device_type"]
            elif query == self.READ_TEMP_LIMIT:
                return self.state["info"]["temp_limit"]
            elif query == self.READ_ACTUAL_SPEED:
                return round(uniform(10, 100), 2)
            elif query == self.READ_ACTUAL_PROCESS_TEMP:
                return round(uniform(15, 100), 2)
            elif query == self.READ_ACTUAL_SURFACE_TEMP:
                return round(uniform(80, 120), 2)
            elif query == self.READ_ACTUAL_FLUID_TEMP:
                return round(uniform(20, 110), 2)
            elif query == self.READ_SURFACE_TEMP_SETPOINT:
                return self.state["surface_temp"]["setpoint"]
            elif query == self.READ_PROCESS_TEMP_SETPOINT:
                return self.state["process_temp"]["setpoint"]
            elif query == self.READ_SPEED_SETPOINT:
                return self.state["speed"]["setpoint"]
            elif query == self.READ_SHAKER_STATUS:
                return self.state["speed"]["active"]
            elif query == self.READ_PROCESS_HEATER_STATUS:
...
class Shaker(RealShaker):
    ...
    async def query(self, query):
        ...
        async with self.lock:  # lock releases on CancelledError
            if query == self.READ_DEVICE_NAME:
                return self.state['name']
            elif query == self.READ_SET_TEMPERATURE:
                return self.state['temp']['setpoint']
            elif query == self.READ_ACTUAL_TEMPERATURE:
                return self.state['temp']['actual']
            elif query == self.READ_HEATER_STATUS:
